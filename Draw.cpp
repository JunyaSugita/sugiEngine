...
void Draw::TriangleInisialize(ID3D12Device* device)
{
	// 頂点データ
	XMFLOAT3 vertices[] = {
		{ -0.5f, -0.5f, 0.0f }, // 左下
	...
	TriangleCount = _countof(vertices);
...
void Draw::GraphInisialize(ID3D12Device* device)
{
...
void Draw::TriangleUpdate(ID3D12GraphicsCommandList* commandList)
{
	// ビューポート設定コマンド
	...
	// 描画コマンド
	commandList->DrawInstanced(TriangleCount, 1, 0, 0); // 全ての頂点を使って描画
}

void Draw::GraphUpdate(ID3D12GraphicsCommandList* commandList)
{
	// ビューポート設定コマンド
	D3D12_VIEWPORT viewport{};
	viewport.Width = WINDOW_WIDTH;
	...
	// 描画コマンド
	commandList->DrawIndexedInstanced(GraphCount, 1, 0, 0, 0); // 全ての頂点を使って描画
